...
public class GAMModel extends Model<GAMModel, GAMModel.GAMParameters, GAMModel.GAMModelOutput> {
  ...
  TwoDimTable genCoefficientTable(String[] colHeaders, double[] coefficients,
                                  String[] coefficientNames, boolean standardized, String tableHeader) {
    ...
    for (int ind = 0; ind < nCoeff; ind++) {
      coeffNames[ind] = coefficientNames[ind] +"_standardized";
    }
    TwoDimTable table = new TwoDimTable(tableHeader, "", coeffNames, colHeaders, colTypes, colFormat,
            "names");
    fillUpCoeffs(coeffNames, coefficients, table, 0);
    return table;
  }
  
  TwoDimTable genCoefficientTableMultinomial(String[] colHeaders, double[][] coefficients, 
                                                     String[] coefficientNames, boolean standardized, String tableHeader) {
    String[] colTypes = new String[]{"string", "double"};
    String[] colFormat = new String[]{"", "%5f"};
    ...
    for (int classInd=0; classInd < nclass; classInd++){
      for (int ind=0; ind < nCoeff; ind++) {
        coeffNames[coeffCounter++] = standardized?coefficientNames[ind]+"_class_"+classInd+"_standardized":coefficientNames[ind]+"_class_"+classInd;
      }
    }
    TwoDimTable table = new TwoDimTable(tableHeader, "", coeffNames, colHeaders, colTypes, colFormat,
            "names");
    for (int classInd=0; classInd<nclass; classInd++)
      fillUpCoeffs(coeffNames, coefficients[classInd], table, classInd*nCoeff);
    return table;
  }
...
//        case multinomial: // should not be used
  ...
  public static class GAMModelOutput extends Model.Output {
    ...
    public TwoDimTable _standardized_coefficients_table;
    public TwoDimTable _coefficients_table_no_centering;
    public TwoDimTable _standardized_coefficients_table_no_centering;
